...
/* Gabriel added */
let EventTestActor = exports.EventTestActor = protocol.ActorClass({
  ...
  events: {
    "event-test-websocket" : {
      type: "event-test-websocket",
      time: Arg(0, "number") // 333
    }
  },
  ...
  start: method(function() {
    if (!this._observerAdded) {
      Services.obs.addObserver(this, 'event-test-websocket', false);
  ...
  stop: method(function() {
    if (this._observerAdded) {
      Services.obs.removeObserver(this, 'event-test-websocket');
  ...
  observe: function (subject, topic, data) {
    if (topic == "event-test-websocket") {
      console.logStringMessage(data);
      // Forward event loop lag event
      // events.emit(this, "event-test-websocket", data);
    }
  },
